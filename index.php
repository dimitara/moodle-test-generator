...
admin_externalpage_setup('reportpdfquizgen', '', null, '', array('pagelayout'=>'report'));
echo $OUTPUT->header();

// Log.
\report_pdfquizgen\event\report_viewed::create(array('other' => array('requestedqtype' => $requestedqtype)))->trigger();
...
if ($requestedqtype) {
    ...
    if ($requestedqtype == 'missingtype') {
        $title = get_string('reportformissingqtypes', 'report_pdfquizgen');
        ...
        $title = get_string('reportforallqtypes', 'report_pdfquizgen');

        $sqlqtypetest = '';
        $params = array();

    } else {
        $title = get_string('reportforqtype', 'report_pdfquizgen',
    ...
    $table->head = array(
            ...
            get_string('totalquestions', 'report_pdfquizgen'),
            get_string('visiblequestions', 'report_pdfquizgen'),
            get_string('hiddenquestions', 'report_pdfquizgen'));
    $table->data = array();
    $table->class = '';
    $table->id = '';
    ...
    foreach ($counts as $count) {
        ...
        if ($url) {
            $contextname = '<a href="' . $url . '" title="' .
                    get_string('editquestionshere', 'report_pdfquizgen') .
                    '">' . $contextname . '</a>';
        }
